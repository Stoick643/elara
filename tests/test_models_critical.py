...
class TestGoalProgressCalculation:
    ...
    def test_goal_progress_updates_with_task_completion(self, app, test_goal):
        ...
        with app.app_context():
            ...
            task1 = Task(user_id=goal.user_id, goal_id=goal.id, title="Task 1")
            task2 = Task(user_id=goal.user_id, goal_id=goal.id, title="Task 2")
            task3 = Task(user_id=goal.user_id, goal_id=goal.id, title="Task 3")
            ...
            # Complete 2nd task = 67% progress (2/3 = 66.67% rounds to 67)
            task2.mark_complete()
            progress = goal.calculate_progress()
            assert progress == 67
    ...
    def test_task_completion_auto_updates_goal_progress(self, app, test_goal):
        ...
        with app.app_context():
            goal = get_goal(test_goal)
            task = Task(user_id=goal.user_id, goal_id=goal.id, title="Auto-update test")
            db.session.add(task)
            db.session.commit()
            ...
            assert goal.progress == 100  # 1 out of 1 task completed
    ...
    def test_goal_completion_sets_status_and_date(self, app, test_goal):
        ...
        with app.app_context():
            ...
            assert goal.completed_at >= before_completion
...
class TestHabitStreakCalculation:
    ...
    def test_habit_consecutive_checkins_increment_streak(self, app, test_habit):
        ...
        with app.app_context():
            ...
            habit.update_streak(today - timedelta(days=2))
            assert habit.streak_count == 1

            # Day 2 (consecutive)
            habit.last_completed = today - timedelta(days=2)
            habit.update_streak(today - timedelta(days=1))
            assert habit.streak_count == 2

            # Day 3 (consecutive)
            habit.last_completed = today - timedelta(days=1)
            habit.update_streak(today)
    ...
    def test_habit_missed_day_resets_streak(self, app, test_habit):
        ...
        with app.app_context():
            ...
            habit.best_streak = 3
            habit.last_completed = today - timedelta(days=3)

            # Check in after missing 2 days (should reset)
            habit.update_streak(today)

            assert habit.streak_count == 1  # Reset to 1
            assert habit.best_streak == 3   # Best streak preserved
    ...
    def test_habit_prevents_duplicate_daily_checkins(self, app, test_habit):
        ...
        with app.app_context():
            ...
            success1 = habit.check_in_today()
            assert success1 is True
            assert habit.streak_count == 1

            # Second check-in same day should fail
            success2 = habit.check_in_today()
            assert success2 is False
            assert habit.streak_count == 1  # Unchanged
    ...
    def test_habit_streak_emoji_changes_with_progress(self, app, test_habit):
        ...
        with app.app_context():
            ...
            # Medium streak
            habit.streak_count = 15
            assert habit.get_streak_emoji() == "💪"

            # Long streak
            habit.streak_count = 50
            assert habit.get_streak_emoji() == "🏆"
...
class TestTaskGoalLinking:
    ...
    def test_task_can_unlink_from_goal(self, app, test_goal):
        ...
        with app.app_context():
            ...
            assert task not in goal.tasks.all()
...
class TestValueGoalHierarchy:
    ...
    def test_value_goal_relationship(self, app, test_value):
        ...
        with app.app_context():
            value = get_value(test_value)
            goal1 = Goal(user_id=value.user_id, value_id=value.id, title="Goal 1")
            goal2 = Goal(user_id=value.user_id, value_id=value.id, title="Goal 2")

            db.session.add_all([goal1, goal2])
            db.session.commit()

            # Test relationship from both sides
            assert goal1.value == value
            assert goal2.value == value
            assert goal1 in value.goals.all()
            assert goal2 in value.goals.all()
            assert value.goals.count() == 2
...
class TestUserPersonalitySystem:
    ...
    def test_user_personality_selection(self, app, test_user):
        ...
        with app.app_context():
            ...
            assert user.avatar_personality == "friend"

            # Change to different personality
            user.avatar_personality = "champion"
            db.session.commit()

            # Reload and verify
            db.session.refresh(user)
            assert user.avatar_personality == "champion"
...
class TestJournalMoodTracking:
    ...
    def test_journal_mood_emoji_accuracy(self, app, test_user):
        ...
        with app.app_context():
            ...
            entry_sad = JournalEntry(user_id=user.id, content="Sad day", mood_score=2)
            entry_happy = JournalEntry(user_id=user.id, content="Great day", mood_score=9)
            entry_neutral = JournalEntry(user_id=user.id, content="OK day", mood_score=5)
    ...
    def test_journal_mood_color_coding(self, app, test_user):
        ...
        with app.app_context():
            user = get_user(test_user)
            entry_low = JournalEntry(user_id=user.id, content="Low mood", mood_score=3)
            entry_mid = JournalEntry(user_id=user.id, content="Mid mood", mood_score=6)
            entry_high = JournalEntry(user_id=user.id, content="High mood", mood_score=8)
